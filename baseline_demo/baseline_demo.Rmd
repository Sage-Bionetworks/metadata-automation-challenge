...
The **caDSR export** table can be accessed and downloaded from Synapse [**here**](https://www.synapse.org/#!Synapse:syn20540628). Alternatively, I can use the **`synapser`** package to fetch the data via the Synapse API.
...
cadsr_df <- readr::read_tsv(cadsr_entity$path, col_names = T)
```
...
The **caDSR Synthetic data** input table, `table-125350.3093307.tsv`, can be accessed and downloaded from Synapse [**here**](https://www.synapse.org/#!Synapse:syn21088795). Alternatively, I can use
...
anno_dataset_entity <- synapser::synGet(dataset_ids[[dataset_name]], 
...
```{r}
input_df <- readr::read_tsv(dataset_entity$path, col_names = T)
...
```{r}
n_distinct(demo_col_ov_pv_hits$CDE_ID)
```
...
The **`match_ov_pv()`** function performs a basic *inner join* (i.e., looks for exact matches) to find the intersection between unique OVs in the column and expanded PVs in the dataframe I constructed above.
...
The manually annotated DE **!!!FILL IN HERE!!!** is my third best result... so it won't get me max points — but still not bad.
...
```{r}
s <- get_overall_score(submission_data, anno_data)
```
